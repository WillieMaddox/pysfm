        ...
        for j in self.optim_track_ids:
            for i in self.optim_camera_ids:
                if bundle.tracks[j].has_measurement(i):
                    cost += sum(np.square(bundle.residual(i, j)))
        return cost
    ...
    def compute_schur_complement(self):
        ...
        for pos, i in enumerate(self.track_ids):
            if self.SCHUR_COMPLIMENT_PINV_THRESHOLD is None:
                self.HPP_invs[pos] = np.linalg.inv(self.HPPs[pos])  # [alex] testing psuedoinverse
            else:
                self.HPP_invs[pos] = np.linalg.pinv(self.HPPs[pos],
                                          self.SCHUR_COMPLIMENT_PINV_THRESHOLD)  # [alex] testing psuedoinverse
