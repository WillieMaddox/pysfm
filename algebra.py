...
def prdot(H, X):
    ...
    if X.ndim == 1:
        assert len(X) == np.size(H, 1) - 1, 'H.shape was %s, X.shape was %s' % (str(H.shape), str(X.shape))
        return pr(np.dot(H, unpr(X)))
    elif X.ndim == 2:
        assert np.size(X, 1) == np.size(H, 1) - 1, 'H.shape was %s, X.shape was %s' % (str(H.shape), str(X.shape))
