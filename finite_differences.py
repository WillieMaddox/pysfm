...
def numeric_derivative(f, x0, h=1e-8):
    assert np.isscalar(x0), 'numeric_derivative can only deal with scalar inputs'
...
def numeric_hessian(f, x0, h=1e-6):
    x0 = np.atleast_1d(np.asarray(x0, np.float128))
    f0 = np.atleast_1d(np.asarray(f(x0), np.float128))
    ...
    H = np.empty((flen, xlen, xlen), np.float128)
    for i in range(xlen):
        ei = h * axis(i, xlen)
        ...
        for j in range(i):
            ej = h * axis(j, xlen)
            A = np.asarray(f(x0 + ei + ej), np.float128)
            B = np.asarray(f(x0 + ei - ej), np.float128)
            C = np.asarray(f(x0 - ei + ej), np.float128)
            D = np.asarray(f(x0 - ei - ej), np.float128)
            H[:, i, j] = (A + D - B - C) / (4. * h * h)
            H[:, j, i] = H[:, i, j]
...
def check_descent_direction(update, f, x0, h=1e-8):
    f0 = np.squeeze(f(x0))
    assert np.ndim(f0) == 0, 'function must return a scalar'

    # check that the update has the right shape
    assert np.shape(update) == np.shape(x0), 'update had shape %s' % np.shape(update)

    # compute direction of steepest descent at x0
    # note that this is the *negative* gradient
    steepest_descent = -np.squeeze(numeric_jacobian(f, x0, h))
    assert np.ndim(steepest_descent) == 1

    # update *must* have a positive dot product with g
    dp = np.dot(steepest_descent, update) / (np.linalg.norm(steepest_descent) * np.linalg.norm(update))
    assert dp > 0, \
