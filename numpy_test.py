...
class NumpyTestCase(unittest.TestCase):
    ...
    def assertFunctionsEqual(self, f1, f2, near, radius, nsamples, tol=1e-5):
        near = np.atleast_1d(np.asarray(near))
        lo = near - radius
        hi = near + radius
        n = np.ceil(nsamples ** (1. / len(near)))
        samples = [np.linspace(xa, xb, n) for xa, xb in zip(lo, hi)]
    ...
    def assertJacobian(self, f, Jf, x0, tol=1e-5, h=1e-8, verbose=False):
        ...
        if callable(Jf):
            J_analytic = np.atleast_2d(Jf(x0))
        else:
            J_analytic = np.atleast_2d(Jf)
